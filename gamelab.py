...
def minimax_decision(state, game):
    ...
    player = game.to_move(state) ##  move player to the new state
    
    def max_value(state):
        ### ... you fill this in ...
        ##if Terminal-test[game](state) is true, then return utility[game](state)
        value=-infinity
        if game.terminal_test(state):
            return game.utility(state,player)
         #   #else if Max is to move in state then return higehst minimax-value of successors(state)
        for move,op in game.successors(state):
            value=max(value,min_value(op))
        return value
    ...
    def min_value(state):
        ...
        if game.terminal_test(state):
            return game.utility(state,player)
            #else if Min is to move in state then return lowest minimax-value of successors(state)
        for move,op in game.successors(state):
            value=min(value,max_value(op))
        return value
...
def alphabeta(state, game):
    ...
    def max_value(state, alpha, beta):
        ...
        for move,op in game.successors(state):
            value=max(value,min_value(op,alpha,beta))
            if value>= beta:
                return value
    ...
    def min_value(state, alpha, beta):
        ...
        for move,op in game.successors(state):
            value=min(value,max_value(op,alpha,beta))
            if value<=alpha:
                return value
                beta=min(beta,value)
        return value
...
def alphabeta_cutoff(state, game, d=4, eval_fn=None):
    ...
    def max_value(state, alpha, beta, depth):
        ...
        if game.terminal_test(state):
            return game.utility(state,player)
        for move,op in game.successors(state):
            value=max(value,min_value(op,alpha,beta,depth))
            if value>= beta:
                return value
                alpha=max(alpha,value)
        return value
    ...
    def min_value(state, alpha, beta, depth):
        ...
        for move,op in game.successors(state):
            value=min(value,max_value(op,alpha,beta,depth))
            if value<=alpha:
                return value
                beta=min(beta,value)
        return value
...
class GameInterface:
    ...
    def draw(self):
        ...
        for i in range(0,500,500/self.game.v):
            self.canvas.create_line(0,i,500,i);
        for i in range(0,500,500/self.game.h):
    ...
    def click( self, event ):
        ...
        self.move = (event.x/(500/self.game.h)+1,event.y/(500/self.game.v)+1)
        print self.move
    ...
    def processEndGame(self):
        ...
        if utility != 0:
            ...
            mins = map(min,zip(*line))
            maxs = map(max,zip(*line))
